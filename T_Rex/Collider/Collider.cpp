...
Collider::Collider()
	: bDrawColliderColor(false), bDrawCollider(false)
{
...
void Collider::CreateBuffer()
{
	...
	// Create VertexBuffer
	{
		...
		// UpdateSubresource 함수는 버퍼를 수정한다.
...
bool Collider::Obb(Sprite * a, Sprite * b)
{
	...
	float lengthB = SeparateAxis(A.Right, B.Length_Right, B.Length_Up);
	float length = fabsf(D3DXVec2Dot(&A.Right, &distance));
	if (length > lengthA + lengthB) return false;

	// A.Up축을 기준으로 검사
	lengthA = D3DXVec2Length(&A.Length_Up);
	lengthB = SeparateAxis(A.Up, B.Length_Right, B.Length_Up);
	length = fabsf(D3DXVec2Dot(&A.Up, &distance));
	if (length > lengthA + lengthB) return false;

	// B.Right축을 기준으로 검사
	lengthB = D3DXVec2Length(&B.Length_Right);
	lengthA = SeparateAxis(B.Right, A.Length_Right, A.Length_Up);
	length = fabsf(D3DXVec2Dot(&B.Right, &distance));
	if (length > lengthA + lengthB) return false;

	// B.Up축을 기준으로 검사
	lengthB = D3DXVec2Length(&B.Length_Up);
	lengthA = SeparateAxis(B.Up, A.Length_Right, A.Length_Up);
...
void Collider::Update(D3DXMATRIX & V, D3DXMATRIX & P)
{
	...
	// collider를 그릴지 안그릴지 테스트 코드 지울 예정
...
bool Collider::AabbLine(D3DXMATRIX& world1, D3DXMATRIX& world2)
{
	D3DXVECTOR2 position1 = D3DXVECTOR2(world1._41, world1._42);
	D3DXVECTOR2 position2 = D3DXVECTOR2(world2._41, world2._42);
	// fabsf 는 실수의 절대값
